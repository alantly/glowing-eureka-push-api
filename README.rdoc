...
== Description
Encrypt notifications and send to GCM built using express. Service will take the payload and encrypt it based on the Push Notification specification using keys provided. Once completed, the service will push the encrypted payload to gcm with the specific user device endpoint.
...
=== Sample Payload to localhost:4000/notification
  {
      "subscription": {
